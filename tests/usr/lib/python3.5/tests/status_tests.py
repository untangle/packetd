...
def get_status(call):
    ...
    result = subprocess.run('curl -m 5 -X GET -s -o - -H "Content-Type: application/json; charset=utf-8" "http://localhost/api/status/%s"' % call, shell=True, stdout=subprocess.PIPE)
