...
func PluginConntrackHandler(message int, entry *dispatch.ConntrackEntry) {
	logger.LogDebug(logsrc, "ConntrackHandler MSG:%c ID:%d PROTO:%d SADDR:%s SPORT:%d DADDR:%s DPORT:%d TX:%d RX:%d UC:%d\n",
		message,
		entry.ConntrackID,
...
func PluginNetloggerHandler(netlogger *dispatch.NetloggerMessage) {
	logger.LogDebug(logsrc, "NetloggerHandler PROTO:%d ICMP:%d SIF:%d DIF:%d SADR:%s DADR:%s SPORT:%d DPORT:%d MARK:%X PREFIX:%s\n",
		netlogger.Protocol,
		netlogger.IcmpType,
